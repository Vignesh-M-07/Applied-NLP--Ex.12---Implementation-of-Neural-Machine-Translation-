...
class Translator(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        # Pass the encoded context and the target to the decoder
...
def rouge1_similarity(candidate, reference):
    ...
    for token in candidate_word_counts.keys():
        ...
        # Update the overlap by getting the minimum between the two token counts
...
def average_overlap(samples, similarity_fn):
    ...
    for index_candidate, candidate in enumerate(samples):    
        ...
        ### START CODE HERE ###
        overlap = 0  # Initially overlap is zero
